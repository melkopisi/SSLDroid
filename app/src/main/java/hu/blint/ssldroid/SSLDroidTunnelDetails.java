...
public class SSLDroidTunnelDetails extends Activity {
  ...
  @Override protected void onCreate(Bundle bundle) {
    ...
    checkPermission();
    Button confirmButton = findViewById(R.id.tunnel_apply_button);
    name = findViewById(R.id.name);
    localport = findViewById(R.id.localport);
    remotehost = findViewById(R.id.remotehost);
    remoteport = findViewById(R.id.remoteport);
    pkcsfile = findViewById(R.id.pkcsfile);
    pkcspass = findViewById(R.id.pkcspass);
    ...
    rowId = (bundle == null) ? null : (Long) bundle.getSerializable(SSLDroidDbAdapter.KEY_ROWID);
    if (extras != null) {
  ...
  private void showFiles(final List<File> names, final File baseurl) {
    ...
    new AlertDialog.Builder(SSLDroidTunnelDetails.this).setTitle(R.string.pkcsfile_pick)
  ...
  private void pickFileSimple() {
    ...
    if (!Environment.MEDIA_MOUNTED_READ_ONLY.equals(state) && !Environment.MEDIA_MOUNTED.equals(
        state)) {
      new AlertDialog.Builder(SSLDroidTunnelDetails.this).setMessage(R.string.alert_sdcard_absent)
          .setNegativeButton(android.R.string.cancel, null)
          .create()
          .show();
      return;
  ...
  private void populateFields() {
    if (rowId != null) {
      ...
      if (!doClone) {
        name.setText(Tunnel.getString(Tunnel.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_NAME)));
        localport.setText(
            Tunnel.getString(Tunnel.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_LOCALPORT)));
      }
      remotehost.setText(
          Tunnel.getString(Tunnel.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_REMOTEHOST)));
      remoteport.setText(
          Tunnel.getString(Tunnel.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_REMOTEPORT)));
      pkcsfile.setText(
          Tunnel.getString(Tunnel.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_PKCSFILE)));
      pkcspass.setText(
          Tunnel.getString(Tunnel.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_PKCSPASS)));
    }
  }
      ...
      while (eAliases.hasMoreElements()) {
        ...
        if (myStore.isKeyEntry(strAlias)) {
          ...
          try {
            ...
            Toast.makeText(getBaseContext(), "PKCS12 problem: " + e.getMessage(), Toast.LENGTH_LONG)
                .show();
            return false;
          }
        }
  ...
  private void saveState() {
    ...
    if (rowId == null || doClone) {
      long id =
          dbHelper.createTunnel(sName, sLocalport, sRemotehost, sRemoteport, sPkcsfile, sPkcspass);
      if (id > 0) {
        rowId = id;
      }
    } else {
      dbHelper.updateTunnel(rowId, sName, sLocalport, sRemotehost, sRemoteport, sPkcsfile,
          sPkcspass);
    }
  ...
  private final class SSLDroidTunnelHostnameChecker extends AsyncTask<String, Integer, Boolean> {
    ...
    protected void onPostExecute(Boolean result) {
      if (result == false) {
        Toast.makeText(getBaseContext(), "Remote host not found, please recheck...",
  ...
  private final class SSLDroidTunnelValidator implements View.OnClickListener {
    public void onClick(View view) {
      if (name.getText().length() == 0) {
        Toast.makeText(getBaseContext(), "Required tunnel name parameter not set up, skipping save",
            Toast.LENGTH_LONG).show();
        return;
      }
      //local port validation
      if (localport.getText().length() == 0) {
        Toast.makeText(getBaseContext(), "Required local port parameter not set up, skipping save",
            Toast.LENGTH_LONG).show();
        return;
      } else {
        ...
        try {
          ...
          Toast.makeText(getBaseContext(), "Local port parameter has invalid number format",
              Toast.LENGTH_LONG).show();
          return;
        }
        if (cPort < 1025 || cPort > 65535) {
          Toast.makeText(getBaseContext(), "Local port parameter not in valid range (1025-65535)",
              Toast.LENGTH_LONG).show();
          return;
        }
        ...
        while (cursor.moveToNext()) {
          String cDbName =
              cursor.getString(cursor.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_NAME));
          int cDbPort =
              cursor.getInt(cursor.getColumnIndexOrThrow(SSLDroidDbAdapter.KEY_LOCALPORT));
          if (cPort == cDbPort && !cDbName.contentEquals(name.getText().toString())) {
            Toast.makeText(getBaseContext(),
                "Local port already configured in tunnel '" + cDbName + "', please change...",
                Toast.LENGTH_LONG).show();
            return;
          }
        }
      ...
      if (remotehost.getText().length() == 0) {
        Toast.makeText(getBaseContext(), "Required remote host parameter not set up, skipping save",
            Toast.LENGTH_LONG).show();
        return;
      } else {
      ...
      if (remoteport.getText().length() == 0) {
        Toast.makeText(getBaseContext(), "Required remote port parameter not set up, skipping save",
            Toast.LENGTH_LONG).show();
        return;
      } else {
        ...
        try {
          ...
          Toast.makeText(getBaseContext(), "Remote port parameter has invalid number format",
              Toast.LENGTH_LONG).show();
          return;
        }
        if (cPort < 1 || cPort > 65535) {
          Toast.makeText(getBaseContext(), "Remote port parameter not in valid range (1-65535)",
              Toast.LENGTH_LONG).show();
          return;
        }
      }
