...
public class SSLDroidDbHelper extends SQLiteOpenHelper {
  ...
  private static final String DATABASE_CREATE =
      "CREATE TABLE IF NOT EXISTS tunnels (_id integer primary key autoincrement, "
  ...
  private static final String STATUS_CREATE =
      "CREATE TABLE IF NOT EXISTS status (name text, value text);";
  ...
  @Override public void onUpgrade(SQLiteDatabase database, int oldVersion, int newVersion) {
    Log.w(SSLDroidDbHelper.class.getName(), "Upgrading database from version "
        + oldVersion
        + " to "
        + newVersion
        + ", which will add a status table");
